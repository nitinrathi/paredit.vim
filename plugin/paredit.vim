...
function! PareditInitBuffer()
    ...
    if &ft =~ '.*\(clojure\|scheme\|racket\|fennel\).*'
    ...
    if g:paredit_mode
        ...
        if &ft =~ '.*\(clojure\|scheme\|racket\|fennel\).*' && g:paredit_smartjump
...
function! s:ReGatherUp()
    if g:paredit_electric_return && getline('.') =~ '^\s*)'
        ...
        endwhile
        normal! Jl
    elseif g:paredit_electric_return && getline('.') =~ '^\s*\(\]\|}\)' && &ft =~ '.*\(clojure\|scheme\|racket\|fennel\).*' 
...
function! PareditInsertClosing( open, close )
    ...
    if g:paredit_electric_return && newpos[0] > line('.')
        ...
        if len(nextline) > 0 && nextline[0] =~ '\]\|}' && &ft =~ '.*\(clojure\|scheme\|racket\|fennel\).*' 
